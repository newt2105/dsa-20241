 ...
 * @param finalPath Final path if found
 * @return True if path is found, false otherwise
 */
bool dfsHelper(const Graph &graph,
 ...
 * @return Vector of node IDs representing the path, empty if no path exists
 */
vector<string> Algorithms::dfs(const Graph &graph, const string &start, const string &end)
{
    ...
    dfsHelper(graph, start, end, visited, path, finalPath);
    return finalPath;
}
